...
type SjrQuartileValue = typeof SJR_QUARTILES[number];
...
interface SjrDatasetEntry extends SjrQuartileData {
    normalizedTitle: string;
    normalizedWithoutPrefix: string;
}

interface SjrDatasetIndex {
    exact: Map<string, SjrDatasetEntry[]>;
    buckets: Map<string, SjrDatasetEntry[]>;
    entries: SjrDatasetEntry[];
}

const sjrLookupCache = new Map<string, SjrLookupCacheEntry>();
let sjrDatasetPromise: Promise<SjrDatasetIndex> | null = null;

function getRuntimeResourceUrl(path: string): string {
...
function parseSemicolonSeparatedCsv(text: string): string[][] {
    const rows: string[][] = [];
...
function buildBucketKey(value: string, length: number): string {
...
function addToBucket(map: Map<string, SjrDatasetEntry[]>, key: string, entry: SjrDatasetEntry): void {
    const bucketKey = key || '#';
...
function buildNormalizedVariants(normalized: string): string[] {
...
async function loadSjrDataset(): Promise<SjrDatasetIndex> {
    if (!sjrDatasetPromise) {
        sjrDatasetPromise = (async () => {
            const exactMatches = new Map<string, SjrDatasetEntry[]>();
            const bucketMap = new Map<string, SjrDatasetEntry[]>();
            const entries: SjrDatasetEntry[] = [];
            const entryLookup = new Map<string, SjrDatasetEntry>();
            ...
            for (const year of SJR_AVAILABLE_YEARS) {
                ...
                try {
                    ...
                    for (const row of rows) {
                        ...
                        if (!quartileCandidate || !SJR_QUARTILES.includes(quartileCandidate as SjrQuartileValue)) {
                            continue;
                        }
                        const quartile = quartileCandidate as SjrQuartileValue;
...
function collectSjrCandidates(variants: string[], dataset: SjrDatasetIndex): SjrDatasetEntry[] {
    const candidateSet = new Set<SjrDatasetEntry>();
...
function pickBestEntry(entries: SjrDatasetEntry[], publicationYear: number | null | undefined): SjrDatasetEntry | null {
    if (entries.length === 0) {
        return null;
    }

    let bestEntry: SjrDatasetEntry | null = null;
...
async function resolveSjrQuartile(journalName: string, publicationYear?: number | null): Promise<SjrQuartileLookupResult> {
    ...
    let bestCandidate: SjrDatasetEntry | null = null;
    let bestScore = 0;

    for (const candidate of pool) {
        const candidateKeys = [candidate.normalizedTitle, candidate.normalizedWithoutPrefix].filter(Boolean) as string[];
        for (const variant of variants) {
