...
async function saveCachedData(userId, rankCounts, publicationRanks) {
    const cacheKey = getCacheKey(userId);
    const dataToStore = {
        rankCounts,
        publicationRanks,
...
async function fetchVenueAndYear(publicationUrl) {
    ...
    try {
        ...
        if (!response.ok) { /* console.warn(`Failed to fetch ${publicationUrl}: ${response.statusText}`); */
...
function displaySummaryPanel(rankCounts, currentUserId, initialCachedPubRanks) {
    document.getElementById(STATUS_ELEMENT_ID)?.remove();
    document.getElementById(SUMMARY_PANEL_ID)?.remove();
    disconnectPublicationTableObserver();
    ...
    footerDiv.appendChild(reportBugLink);
...
async function main() {
    ...
    try {
        statusElement.querySelector('.gsr-status-text').textContent = "Expanding publications list...";
        ...
        document.querySelectorAll('tr.gsc_a_tr').forEach(row => {
            ...
            if (linkEl instanceof HTMLAnchorElement && linkEl.href && linkEl.textContent) {
                publicationLinkElements.push({
                    url: linkEl.href, rowElement: row,
            ...
            try {
                for (const keyword of IGNORE_KEYWORDS) {
                    if (pubInfo.titleText.includes(keyword))
                        return { rank: "N/A", rowElement: pubInfo.rowElement, titleText: pubInfo.titleText };
                }
                ...
                console.warn(`Error processing ${pubInfo.url}:`, error);
            }
